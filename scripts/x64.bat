...
cmake -G %VC_CMAKE% %CIOSNATIONS% -DCMAKE_INSTALL_PREFIX=%COMPILEPATH%\%TARGET% -DCMAKE_CONFIGURATION_TYPES=%BTYPE% -DCMAKE_BUILD_TYPE=%BTYPE% -DBUILD_SHARED_LIBS=%DOSHARED%
...
del /Q %CIOSNATIONS%\Packages\%TARGET%.7z
move /Y %TARGET%.7z %CIOSNATIONS%\Packages
rd /S /Q %TARGET%
